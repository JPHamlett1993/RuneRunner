...
public class MineCopper extends Task<ClientContext> {

    public MineCopper(ClientContext ctx) {
        super(ctx);
    }
    ...
    @Override
    public void execute() {
        GameObject rock = ctx.objects.select().id(Constants.getCopperIds()).nearest().poll();
        rock.interact("Mine");
